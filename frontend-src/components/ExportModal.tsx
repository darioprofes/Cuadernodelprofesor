...
const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, classes, courses, keyCompetences, specificCompetences, evaluationCriteria, programmingUnits, basicKnowledge, academicConfiguration }) => {
    ...
    const handleExport = () => {
        ...
        for (const classData of classesToExport) {
            ...
            const classNameSanitized = buildClassName(classData.grupo, course.subject).replace(/ /g, '_');
            const repartoIgualCriterios = !course.pesoCriteriosManual;
            const { evaluationPeriods } = academicConfiguration;

            // 1. Gradebook CSV — la nota oficial (motor de criterios) junto a la
            // de comparación por categorías (sistema tradicional), para que quede
            // trazable cuál es cuál en el informe exportado.
            const gradebookHeaders = [
                'Alumn@',
                ...evaluationPeriods.flatMap(p => [`${p.name} (criterios)`, `${p.name} (cat.)`]),
            ...
            const gradebookRows = classData.students.map(student => {
                ...
                evaluationPeriods.forEach(p => {
                    ...
                    row.push(oficial.grade?.toFixed(2) ?? '');
                    row.push(comparacion.grade?.toFixed(2) ?? '');
                });
                const finalOficial = calculateFinalGradeCriterial(student.id, classData, classCriteria, repartoIgualCriterios);
                const finalComparacion = calculateOverallFinalGradeForStudent(student.id, classData, academicConfiguration);
                row.push(finalOficial.grade?.toFixed(2) ?? '');
                row.push(finalComparacion.grade);
                return row;
            });
                        ...
                        {classes.map(c => (
                            <label key={c.id} className="flex items-center space-x-2 p-2 hover:bg-slate-50 rounded-md">
                                <input
                                    ...
                                    className={checkboxClassName}
                                />
                                <ClassLabel classData={c} courses={courses} />
                            </label>
                        ))}
                    </div>
                </div>
