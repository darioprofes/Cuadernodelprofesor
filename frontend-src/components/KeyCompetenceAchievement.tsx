...
import type { ClassData, SpecificCompetence, KeyCompetence, EvaluationCriterion, AcademicConfiguration, Student, Course } from '../types';
...
const KeyCompetenceAchievement: React.FC<KeyCompetenceAchievementProps> = ({ classData, courses, competences, keyCompetences, criteria, academicConfiguration }) => {
    const [selectedPeriodId, setSelectedPeriodId] = useState<string>('all');
    const { evaluationPeriods } = academicConfiguration;
    const [drilldownData, setDrilldownData] = useState<DrilldownData | null>(null);
    const repartoIgual = !courses.find(c => c.id === classData.courseId)?.pesoCriteriosManual;

    const studentKeyCompetenceGrades = useMemo(() => {
        const studentGrades = new Map<string, Map<string, number | null>>();
        for (const student of classData.students) {
            studentGrades.set(student.id, calculateStudentKeyCompetenceGrades(student.id, classData, criteria, competences, keyCompetences, repartoIgual, selectedPeriodId === 'all' ? undefined : selectedPeriodId));
        }
        return studentGrades;
    }, [classData, criteria, competences, keyCompetences, repartoIgual, selectedPeriodId]);
    ...
    const handleCellClick = (student: Student, keyCompetence: KeyCompetence) => {
        ...
        const studentScGrades = calculateStudentCompetenceGrades(student.id, classData, criteria, specificCompetencesForKeyComp, repartoIgual, selectedPeriodId === 'all' ? undefined : selectedPeriodId);
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border p-4 sm:p-5 overflow-hidden">
            <div className="flex justify-between items-center mb-4">
                <h2 className={TYPOGRAPHY.pageTitle}>Grado de Consecución de Competencias Clave</h2>
                <div>
                    <Select
