...
import type { ClassData, Course, Assignment, AcademicConfiguration } from '../types';
...
const CopyAssignmentModal: React.FC<CopyAssignmentModalProps> = ({
    ...
    useEffect(() => {
        ...
        // selectedClassId excluded on purpose: the parent only mounts this modal
        // (GradebookTable: {assignmentToCopy && <CopyAssignmentModal .../>}), so
        // this effect only ever runs once per fresh instance, when selectedClassId
        // still holds its useState(currentClassId) initial value. Adding it as a
        // dep would make this re-run whenever the user picks a different target
        // class, clobbering the category the second effect below just set.
