...
El presente documento constituye la guía técnica de referencia integral para la configuración, administración, explotación y mantenimiento de la aplicación **Cuaderno del Profesorado**. Esta herramienta ha sido diseñada bajo el principio de **soberanía de datos**: toda la información sensible del alumnado (datos personales, necesidades educativas y calificaciones) y del proceso evaluativo se persiste en un servidor propio del centro o del docente (no en servicios de terceros ni en la nube de ningún proveedor comercial), evitando suscripciones recurrentes y los riesgos de privacidad asociados a la cesión de datos a terceros.

> **Nota sobre esta variante**: este repositorio es un fork del [CuadernMestre v1.0](https://github.com/elCordones/CuadernMestre-v1.0) original, que sí guardaba los datos únicamente en el propio navegador (IndexedDB), sin ningún servidor. Esta variante sustituye ese almacenamiento local por un backend mínimo propio (ver `api/` en la raíz del repositorio) que persiste el mismo blob de datos en una base de datos Postgres — pensado para poder acceder desde varios dispositivos y hacer copias de seguridad centralizadas, a costa de necesitar desplegar ese backend (ver el README raíz del repositorio).
...
Tras el despliegue exitoso de la aplicación (en local para desarrollo, o en el servidor del centro/docente en producción), es imperativo realizar una adaptación meticulosa del entorno de trabajo antes de proceder a la introducción de datos académicos. Una configuración inicial precisa no solo es estética, sino fundamental para garantizar la automatización correcta de los cálculos de evaluación ponderada y la integridad de la planificación temporal a lo largo del curso.
...
Al tratarse de una aplicación basada en tecnologías web modernas (React/Node.js), pueden surgir incidencias puntuales de renderizado debido a la gestión de memoria del navegador.
