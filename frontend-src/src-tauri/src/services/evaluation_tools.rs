...
const COLUMNS: &str = "id, type, name, levels, items";

fn row_to_json(row: &Row) -> rusqlite::Result<Value> {
    let levels: String = row.get(3)?;
...
pub fn create(conn: &Connection, body: Value) -> Result<Value, ApiError> {
    ...
    let now = db::now_iso();
    conn.execute(
        "INSERT INTO evaluation_tools (id, type, name, levels, items, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        params![
            id, tool_type, name,
...
pub fn update(conn: &Connection, id: &str, body: Value) -> Result<Value, ApiError> {
    ...
    conn.execute(
        "UPDATE evaluation_tools SET type = ?, name = ?, levels = ?, items = ?, updated_at = ? WHERE id = ?",
        params![
            tool_type, name,
            serde_json::to_string(&levels).map_err(ApiError::internal)?,
            serde_json::to_string(&items).map_err(ApiError::internal)?,
